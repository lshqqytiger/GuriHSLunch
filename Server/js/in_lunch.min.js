...
const date = new Date();
var month = date.getMonth()+1 < 10 ? "0" + date.getMonth()+1 : date.getMonth()+1 // Regardless of type of getMonth()
var day = date.getDate() < 10 ? "0" + date.getDate() : date.getDate() // also regardless of that
const SD_SCHUL_CODE = "7530054"
const ATPT_OFCDC_SC_CODE = "J10"
var MLSV_YMD = `${date.getFullYear()}${month}${day}`
var url = `https://open.neis.go.kr/hub/mealServiceDietInfo?Type=json&SD_SCHUL_CODE=${SD_SCHUL_CODE}&ATPT_OFCDC_SC_CODE=${ATPT_OFCDC_SC_CODE}&MLSV_YMD=${MLSV_YMD}`
...
function drawResult(data){
	if(!data) return alert("NO DATA RETURNED")
	if(JSON.parse(data).RESULT){
		$("#date").html(`${date.getFullYear()}년 ${month}월 ${day}일<p>`)
		$("#loading").hide()
...
function rebuildURL(qmonth, qday){
	loading()
...
function onFail(e){
	alert(`AN ERROR WAS OCCURRED ON GETTING DATA!`)
	return;
}
