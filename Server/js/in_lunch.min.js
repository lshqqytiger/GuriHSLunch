...
function drawResult(data){
	...
	if(JSON.parse(data).RESULT){
		...
		if(originDate.getMonth()+1 == Number(month) && originDate.getDate() == Number(day)) $("#date").html(`${date.getFullYear()}년 ${month}월 ${day}일 ${week}요일 (오늘)<p>`);
		return;
	}
	...
	if(originDate.getMonth()+1 == Number(month) && originDate.getDate() == Number(day)){
...
function reloadLunch(nmonth, nday, reload){
	if(!nmonth || !nday) return getLunch();
	else{
		loading();
